...
public abstract class Graph {
	...
	protected int[][] multiMatrax(int[][] mat)
	{
		...
		for(int i = 0; i< width; ++i)
		{
			...
			{
				result[i][j] = sumAll(mat, i, j);
			}
		}
		return result;
	}
	private int sumAll(int[][] mat, int r, int c)
	{
		int result = 0;
		int width = mat.length;
		for(int i = 0; i< width ; ++i)
		{
			result += mat[r][i] * mat[i][c];
		}
		return result;
	}
	...
	public static void main (String[] args) {
		GraphLoader.createIntersectionsFile("data/maps/myucsd.map", "data/intersections/myucsd1.intersections");
