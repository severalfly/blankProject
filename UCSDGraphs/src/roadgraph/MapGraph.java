...
public class MapGraph
{
	...
	public int getNumEdges()
	{
		...
		int result = 0;
		// 遍历所有的顶点
		for (Entry<GeographicPoint, List<GeographicPoint>> map : this.pointMap.entrySet())
		{
	...
	public void addEdge(GeographicPoint from, GeographicPoint to, String roadName, String roadType, double length) throws IllegalArgumentException
	{
		...
		List<GeographicPoint> neighbors = this.pointMap.get(from);
	...
	public List<GeographicPoint> bfs(GeographicPoint start, GeographicPoint goal, Consumer<GeographicPoint> nodeSearched)
	{
		...
		HashSet<GeographicPoint> vistedSet = new HashSet<>();
		queue.add(start);
		vistedSet.add(start);
		while (!queue.isEmpty())
		{
			GeographicPoint curr = queue.element();
			queue.remove();
			if (curr.equals(goal))
			{
				...
				result.add(curr);
				curr = goal;
				while (parentMap.get(curr) != start)
				{
					result.add(parentMap.get(curr));
			...
			List<GeographicPoint> neighbors = this.pointMap.get(curr);
			//			LinkedList<GeographicPoint> neghbors = 
...
class RoadLineInfo
{
	...
	{
		return l1 - l2 >= 0.00000001 && l1 - l2 <= 0.00000001;
	}
